...
public class WebClientService {
    ...
    public WebClientService(WebClient.Builder webClientBuilder,
                            @org.springframework.beans.factory.annotation.Value("${nba.api.timeout-seconds:30}") int timeoutSeconds) {
    ...
    // Version sans type (retourne String par défaut)
    public Mono<String> performGetCall(Map<String, String> params, String endPoint) {
        return performGetCall(params, endPoint, String.class);
    }

    public <T> Mono<T> performGetCall(Map<String, String> params, String endPoint, Class<T> responseType) {
        ...
        return webClient.get()
                .uri(uriBuilder -> uriBuilder
                        ...
                        .build())
                .retrieve()
                .bodyToMono(responseType)  // ✅ Utilise le type passé en paramètre
                .onErrorResume(WebClientResponseException.class, ex -> {
                    logger.error("Erreur WebClient - Status: {}, Body: {}",
                            ex.getStatusCode(), ex.getResponseBodyAsString());
                    return Mono.error(new RuntimeException("Erreur API NBA: " + ex.getStatusCode() + " - " + ex.getMessage()));
                })
                .onErrorResume(Exception.class, ex -> {
                    logger.error("Erreur générale: ", ex);
                    return Mono.error(new RuntimeException("Erreur serveur: " + ex.getMessage()));
                });
    }
